...
class Str
{
    ...
    public static function snake($value, $delimiter = '_')
    {
        ...
        if (isset($snakeCache[$value][$delimiter])) {
            return $snakeCache[$value][$delimiter];
        }

        if (!ctype_lower($key = $value)) {
    ...
    public static function studly($value)
    {
        ...
        return $studlyCache[$value] = str_replace(' ', '', ucwords(str_replace(['-', '_'], ' ', $value)));
    }
    ...
    {
        static $charsArray, $langSpecificCharsArray;

        if (null === $charsArray && null === $langSpecificCharsArray) {
            /**
        ...
        if (null !== ($langSpecific = $langSpecificCharsArray[$language] ?? null)) {
            $value = str_replace($langSpecific[0], $langSpecific[1], $value);
        }
     ...
     * @return string
     */
    public static function slug($value, $separator = '-', $language = 'en')
    {
        return trim(preg_replace('/[^A-Za-z0-9-]+/', $separator, self::ascii($value, $language)), $separator);
    }
     ...
     * @return string
     */
    public static function replace($search, $replacements, $value)
    {
        if (($count = substr_count($value = str_replace($search, '%s', $value), '%s')) !== ($replacementCount = count($replacements = Arr::wrap($replacements)))) {
            for ($i = 0; $i < ($count - $replacementCount); ++$i) {
                $replacements[] = end($replacements);
            }
        }

        return sprintf($value, ...$replacements);
    }
}