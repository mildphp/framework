...
class PgsqlCompiler extends Compiler
{
    /**
     * @var array
     */
    ...
    protected function compileColumns($builder)
    {
        ...
        if ($builder->distinct) {
            ...
            if (is_array($builder->distinct)) {
                $select .= 'on ('.$this->wrapColumns($builder->distinct).') ';
            }
        }
    ...
    protected function compileBasicWhere($where)
    {
        ...
        if (stripos($where['operator'], 'like') !== false) {
            $column.= '::text';
        }

        return $column.' '.$where['operator'].' '.$this->resolveValueBinding($where['value']);
    }
    ...
    protected function resolveComponentForUpdateOrDelete($builder)
    {
        if ($builder->joins || $builder->limit) {
            ...
            $query = $builder->connection->query()->whereIn('ctid', function ($query) use ($builder) {
                /**
                 * @var Builder $query
                 */
    ...
    protected function compileLock($builder)
    {
        ...
        return $builder->lock ? 'for update' : 'for share';
    }
}